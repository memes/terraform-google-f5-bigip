  ...
  self_links.each_with_index do |url, index|
    ...
    describe params['name'] do
      it 'should meet naming expectations' do
        ...
        expect(instance.name).to match(/#{prefix}-cfe-6nic-minimal-[01]$/)
        # rubocop:disable Layout/LineLength
        expect(instance.hostname).to match(/#{prefix}-cfe-6nic-minimal-[01]\.#{zones[index % zones.length]}\.c\.#{params["project"]}\.internal/)
